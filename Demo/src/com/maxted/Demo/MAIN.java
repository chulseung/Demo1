...
public class MAIN extends  flashbox.ANDROID.DELEGATE
{
	...
	public Hashtable getCustomData(Hashtable dataProvider) {
		try {
			MANAGER.log("getCustomData:"+dataProvider);
			String method=(String)dataProvider.get("method");
			if(method!=null&&method.equals("sampletest")){
	...
	public boolean requestWillStart(){
		...
		if(alert==null){
						...
						alert.setMessage("Wait a minute. Data loading in progress...");
						alert.show();
	...
	public Hashtable doCustomAction(Hashtable action){
		try{
			String method=(String)action.get("method");
			Hashtable extra=(Hashtable)action.get("extra");
			Hashtable param=null;
			Hashtable ret=new Hashtable();
			if(method!=null&&method.equals("addTextField")){
				if(extra!=null){
					param=(Hashtable)extra.get("parameter");
					XTextView view=(XTextView)param.get("text1");
					view.setText("1");
	
				}
			}else if(method!=null&&method.equals("callAction")){
				processActionOfName("accessibleFromNative",null);
			}else if(method!=null&&method.equals("responseAction")){
				ret.put("_state_","CUSTOMSUCCESS");
			}
			return ret;
		}catch(Exception t){
			return null;
